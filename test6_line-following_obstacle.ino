...
int acc_calibration_value = 288;                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 14;                                       //Gain setting for the P-controller (14)
float pid_i_gain = 1.8;                                      //Gain setting for the I-controller (0.7)
float pid_d_gain = 0;                                       //Gain setting for the D-controller (0.65)

//float turning_speed = 20;                                    //Turning speed (20)
//char turning_speed = 20;                                    //Turning speed (20)

float max_target_speed = 10;                                //Max target speed (100)
float deadband = 5;
char max_angle = 20;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
byte start, received_byte, low_bat, obstacle;
...
void loop(){
  if(Serial.available()){                                                   //If there is serial data available
    setpoint = Serial.read();                                          //Load the received serial data in the received_byte variable
    receive_counter = 0;                                                    //Reset the receive_counter variable
  }
  if(receive_counter <= 50)receive_counter ++;                              //The received byte will be valid for 25 program loops (100 milliseconds)
  else setpoint = 127;                                                //After 100 milliseconds the received byte is deleted

//   // устанавливаем курсор в 0-ом столбце, 1 строка (начинается с 0)
//    lcd.setCursor(1, 1);
//    lcd.print(setpoint); // вывод числа секунд после сброса

//    pixels.setPixelColor(0, pixels.Color(0,150,255)); // Moderately bright blue color.
...
*/
  ...
  if(pid_output > 10 || pid_output < -10)pid_error_temp += pid_output * 0.01 ; // 0.015
  ...
  if(pid_output < deadband && pid_output > -deadband)pid_output = 0;                      //Create a dead-band to stop the motors when the robot is balanced

  if(angle_gyro > max_angle || angle_gyro < -max_angle || start == 0 || low_bat == 1){    //If the robot tips over or the start variable is zero or the battery is empty
    pid_output = 0;                                                         //Set the PID controller output to 0 so the motors stop moving
    pid_i_mem = 0;                                                          //Reset the I-controller memory
    start = 0;                                                              //Set the start variable to 0
    self_balance_pid_setpoint = 0;                                          //Reset the self_balance_pid_setpoint variable
    pixels.setPixelColor(0, pixels.Color(70,70,70)); // Moderately bright WHITE color.
    pixels.show(); // This sends the updated pixel color to the hardware.
  }
...
// left, change 'if' statement if hflip = True !
  if(setpoint < -straight_val && start == 1){                                            //If the first bit of the receive byte is set change the left and right variable to turn the robot to the left
    pid_output_left += abs(char(setpoint));                                       //Increase the left motor speed
    pid_output_right -= abs(char(setpoint));                                      //Decrease the right motor speed
    pixels.setPixelColor(0, pixels.Color(0,0,150)); // Moderately bright BLUE color.
    pixels.show(); // This sends the updated pixel color to the hardware.
  }
  
// right,  change 'if' statement if hflip = True !
  if(setpoint > straight_val && setpoint != 127 && start == 1){                                            //If the second bit of the receive byte is set change the left and right variable to turn the robot to the right
    pid_output_left -= abs(char(setpoint));                                       //Decrease the left motor speed
    pid_output_right += abs(char(setpoint));                                      //Increase the right motor speed
    pixels.setPixelColor(0, pixels.Color(0,0,150)); // Moderately bright BLUE color.
    pixels.show(); // This sends the updated pixel color to the hardware.
  }
...
// fwd
  if(setpoint <= straight_val && setpoint >= -straight_val && start == 1){                                            //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
//    if(pid_setpoint > -0.6)pid_setpoint -= 0.001;                            // 0.05 Slowly change the setpoint angle so the robot starts leaning forewards
//    if(pid_output > max_target_speed * -1)pid_setpoint -= 0.001;            //Slowly change the setpoint angle so the robot starts leaning forewards
    pid_setpoint = -0.6;
...
// STOP
  if(setpoint == 127 && start == 1){                                         //Slowly reduce the setpoint to zero if no foreward or backward command is given
    if(pid_setpoint > 0.5)pid_setpoint -=0.1;                              //If the PID setpoint is larger then 0.5 reduce the setpoint with 0.05 every loop
    else if(pid_setpoint < -0.5)pid_setpoint +=0.1;                        //If the PID setpoint is smaller then -0.5 increase the setpoint with 0.05 every loop
    else pid_setpoint = 0;                                                  //If the PID setpoint is smaller then 0.5 or larger then -0.5 set the setpoint to 0
//    pid_output = 0;  // тогда совсем выключит моторы
  ...
  if(pid_setpoint == 0){                                                    //If the setpoint is zero degrees
    if(pid_output < 0)self_balance_pid_setpoint += 0.01;                  //Increase the self_balance_pid_setpoint if the robot is still moving forewards
    if(pid_output > 0)self_balance_pid_setpoint -= 0.01;                  //Decrease the self_balance_pid_setpoint if the robot is still moving backwards
  }
